...
import { RefreshCw, Users, Trophy, Play, AlertCircle } from 'lucide-react';
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Real-time TriviaBattlev5 contract events from Base network
            </CardDescription>
          </div>
          ...
          <div className="space-y-2">
            ...
            {events.sessionStarted.slice(0, 3).map((event, index) => (
              <div key={index} className="text-sm text-muted-foreground">
                Start: {parseInt(event.startTime)} • End: {parseInt(event.endTime)} • {formatTimestamp(event.timestamp)}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            {events.scoreSubmitted.slice(0, 3).map((event, index) => (
              <div key={index} className="text-sm text-muted-foreground">
                Session: {event.sessionId} • Players: {event.playerCount} • {formatTimestamp(event.timestamp)}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-1 text-xs text-muted-foreground">
        ...
        {scoreEvents.slice(0, 2).map((event, index) => (
          <div key={`score-${index}`}>
            ...
            Session {event.sessionId} • {event.playerCount} players {formatTimestamp(event.timestamp)}
          </div>
        ))}
      </div>
    </div>
