...
contract TriviaBattle is ReentrancyGuard, Ownable {
    ...
    event PrizesDistributed(uint256 sessionId, address[] winners, uint256[] amounts);
    ...
    function startSession(uint256 duration) external onlyOwner {
        require(!currentSession.isActive || block.timestamp > currentSession.endTime, "Session already active");
    ...
    function joinBattle() external nonReentrant {
        ...
        require(usdcToken.transferFrom(msg.sender, address(this), ENTRY_FEE), "USDC transfer failed");

        // Transfer platform fee to recipient
        if (platformFee > 0 && platformFeeRecipient != address(0)) {
            require(usdcToken.transfer(platformFeeRecipient, platformFee), "Platform fee transfer failed");
        ...
        currentSession.playerScores[msg.sender] =
            PlayerScore({score: 0, hasJoined: true, hasSubmitted: false, submissionTime: 0});
    ...
    function joinTrialBattle(string calldata sessionId) external {
        ...
        currentSession.trialPlayerScores[sessionId] =
            TrialPlayerScore({score: 0, hasJoined: true, hasSubmitted: false, submissionTime: 0});
    ...
    function distributePrizes() external onlyOwner onlySessionEnded nonReentrant {
        ...
        for (uint256 i = 0; i < currentSession.paidPlayers.length; i++) {
            ...
            if (currentSession.playerScores[player].hasSubmitted) {
                paidPlayerScores[scoreIndex] =
                    ScoreEntry({playerAddress: player, score: currentSession.playerScores[player].score});
                scoreIndex++;
            }
        }
        ...
        emit PrizesDistributed(currentSession.startTime, winners, amounts);
        emit SessionEnded(block.timestamp);
    }
    ...
    function claimWinnings() external nonReentrant {
        ...
        require(usdcToken.transfer(msg.sender, amount), "Claim transfer failed");
    ...
    function getPlayerScore(address player)
        external
        view
        returns (uint256 score, bool hasJoined, bool hasSubmitted, uint256 submissionTime)
    {
     ...
     */
    function getTrialPlayerScore(string calldata sessionId)
        external
        view
        returns (uint256 score, bool hasJoined, bool hasSubmitted, uint256 submissionTime)
    {
