...
var onWeeklyDistribution = (runtime2, payload) => {
...
function readSessionInfo(runtime2, chainSelector, evmConfig) {
  ...
  const isActiveResult = evmClient.callContract(runtime2, {
  ...
  const lastSessionTimeResult = evmClient.callContract(runtime2, {
  ...
  const sessionIntervalResult = evmClient.callContract(runtime2, {
...
function callDistributePrizes(runtime2, chainSelector, evmConfig) {
  ...
  runtime2.log(`Transaction submitted: ${txHash}`);
  return txHash;
}
async function main() {
