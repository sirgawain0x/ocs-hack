...
var onSessionStarted = (runtime2, log) => {
  try {
    ...
    runtime2.log(`Session Started - Session ID: ${decoded.args.sessionId}, Start Time: ${decoded.args.startTime}`);
    return `SessionStarted processed: ${decoded.args.sessionId}`;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    runtime2.log(`Error processing SessionStarted event: ${errorMessage}`);
    return `Error: ${errorMessage}`;
  }
};
var onPlayerJoined = (runtime2, log) => {
  try {
    const topics = log.topics.map((topic) => bytesToHex(topic));
    const data = bytesToHex(log.data);
    const decoded = decodeEventLog({
    ...
    runtime2.log(`Player Joined - Address: ${decoded.args.player}, Session ID: ${decoded.args.sessionId}`);
    return `PlayerJoined processed: ${decoded.args.player}`;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    runtime2.log(`Error processing PlayerJoined event: ${errorMessage}`);
    return `Error: ${errorMessage}`;
  }
};
async function main() {
