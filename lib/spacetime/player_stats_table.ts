...
export class PlayerStatsTableHandle {
  ...
  /**
   * Access to the `playerIdentity` unique index on the table `player_stats`,
   * which allows point queries on the field of the same name
   * via the [`PlayerStatsPlayerIdentityUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.playerStats.playerIdentity().find(...)`.
   *
   * Get a handle on the `playerIdentity` unique index on the table `player_stats`.
   */
  playerIdentity = {
