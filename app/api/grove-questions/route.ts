...
export async function GET(req: NextRequest) {
    try {
        ...
        // Try Grove storage first
        try {
            const { groveStorage } = await import('@/lib/apis/grove');

            if (groveStorage.isConfigured()) {
                try {
                    await groveStorage.initialize();
                    await groveStorage.refreshManifest();

                    // Try to get files from Grove
                    const groveFiles = await groveStorage.listAudioFiles(prefix);

                    if (groveFiles.length === 0) {
                        logger.debug('📁 Grove manifest is empty, populating with local files');
                        await groveStorage.populateManifestFromLocalFiles();
                        const populatedFiles = await groveStorage.listAudioFiles(prefix);

                        if (populatedFiles.length > 0) {
                            // Use local paths since files aren't uploaded yet
                            files = getLocalAudioFiles();
                            source = 'local-grove-pending';
                            logger.info(`📁 Found ${files.length} local audio files (Grove upload pending)`);
                        } else {
                            files = getLocalAudioFiles();
                            source = 'local';
                            logger.info(`📁 Found ${files.length} local audio files`);
                        }
                    } else {
                        files = groveFiles;
                        source = 'grove';
                        logger.info(`📁 Found ${files.length} audio files in Grove`);
                    }
                } catch (groveError) {
                    logger.warn('⚠️ Grove storage failed, falling back to local files:', groveError);
                    files = getLocalAudioFiles();
                    source = 'local';
                    logger.info(`📁 Found ${files.length} local audio files`);
                }
            } else {
                logger.debug('ℹ️ Grove not configured, using local files');
                files = getLocalAudioFiles();
                source = 'local';
                logger.info(`📁 Found ${files.length} local audio files`);
            }
        } catch (importError) {
            logger.warn('⚠️ Failed to import Grove storage, using local files:', importError);
            files = getLocalAudioFiles();
            source = 'local';
            logger.info(`📁 Found ${files.length} local audio files`);
        }
        ...
        for (let i = 0; i < count && i < bag.length; i++) {
            ...
            // Use local path for audio (Grove URLs will be used once files are uploaded)
            const audioUrl = correct.path;
        ...
        logger.error('❌ Error generating Grove questions:', e);
        return NextResponse.json({
